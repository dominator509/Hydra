...
## Test env
Integration tests self-manage schemas on DATABASE_URL; DeepSeek/Anthropic are wiremock fakes — no keys needed for `verify.sh`. `scripts/cache-hit-audit.sh` falls back to the documented local Postgres example when DATABASE_URL is unset so the replay gate can run under local `verify.sh` without extra shell wrapping.
