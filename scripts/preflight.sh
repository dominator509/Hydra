...
for s in install lint format-check typecheck test-unit test-integration test-e2e test-shell-accessibility test-deployment-safety test-operational-tools test-readiness-evidence test-ingress-policy test-smoke-boundary check-ingress-policy test-nightly-conformance test-performance check-release-policy check-egress-policy check-container-image-policy check-observability check-nats-policy backup-scheduler build security-check dependency-audit smoke-test verify cache-hit-audit production-readiness-check check-execplan-state; do
