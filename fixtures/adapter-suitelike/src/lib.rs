//! adapter-suitelike — a second conformance fixture with deliberate quirks.
//!
//! Quirks:
//! - Dates as d/m/Y strings (not ISO)
//! - Paginated inconsistently: page 3 (cursor "3") returns 404
//! - Returns 429 on every 7th upsert call
//! - Unicode names in records (emoji, RTL text)
//! - Idempotent upsert (same as memcrm pattern but with quirks)
//!
//! Exports the same `hydra:bridge/adapter` WIT interface as memcrm.
...
const DESCRIPTOR_NAME: &str = "suitelike";
const DESCRIPTOR_VERSION: &str = "0.2.0";
const RATE_LIMIT_RETRY_AFTER_SECS: u32 = 3;
const BAD_PAGE_CURSOR: &str = "3";
...
fn lock_state() -> Result<MutexGuard<'static, AdapterState>, BridgeError> {
    state()
        .lock()
        .map_err(|_| BridgeError::Upstream("suitelike state poisoned".into()))
}
...
fn next_etag(state: &mut AdapterState) -> String {
    state.next_etag += 1;
    format!("sl-etag-{}", state.next_etag)
}
...
fn default_schema(kind: &str) -> Result<Vec<FieldSchema>, BridgeError> {
    ...
    Ok(vec![
        ...
        // Suitelike quirk: dates are d/m/Y strings
        FieldSchema {
...
impl adapter::Guest for SuiteLike {
    ...
    fn list(kind: String, cursor: Option<String>, limit: u32) -> Result<Page, BridgeError> {
        // Quirk: page 3 (cursor "3") returns 404 inconsistent pagination
        if let Some(ref c) = cursor {
            if c == BAD_PAGE_CURSOR {
                return Err(BridgeError::NotFound(format!(
                    "suitelike inconsistent pagination at cursor {c}"
                )));
            }
        }
    ...
    fn get(kind: String, id: String) -> Result<RawRecord, BridgeError> {
        let state = lock_state()?; // no rate-limit check on get
        ensure_supported_kind(&state.config, &kind)?;
    ...
    fn upsert(rec: RawRecord) -> Result<RawRecord, BridgeError> {
        ...
        // Quirk: every 7th upsert call returns 429
        state.upsert_calls += 1;
        if state.upsert_calls % 7 == 0 {
    ...
    fn changes_since(cursor: String, limit: u32) -> Result<ChangePage, BridgeError> {
        let state = lock_state()?; // no rate-limit check on changes_since
